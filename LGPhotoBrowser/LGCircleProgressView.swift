...
open class LGCircleProgressView: UIView {
    ...
    fileprivate var destinationProgress: Double = 0.0
     ...
     open var progress: Double = 0.000001 {
        didSet {
     ...
     open var refreshRate: Double = 0.0 {
        didSet { setNeedsDisplay() }
    }
    
     open var roundedCap: Bool = false {
        didSet { setNeedsDisplay() }
    }
     ...
     open var trackWidth: CGFloat = 10 {
        didSet { setNeedsDisplay() }
    }
     ...
     open var trackBackgroundColor: UIColor = UIColor.gray {
        didSet { setNeedsDisplay() }
    }
    
     open var trackFillColor: UIColor = UIColor.blue {
        didSet { setNeedsDisplay() }
    }
    
     open var trackBorderColor:UIColor = UIColor.clear {
        didSet { setNeedsDisplay() }
    }
    
     open var trackBorderWidth: CGFloat = 0 {
        didSet { setNeedsDisplay() }
    }
    ...
    @objc func internalInit() {
        let displayLink = CADisplayLink(target: LGImageWeakTarget(target: self),
                                        selector: #selector(LGCircleProgressView.displayLinkTick))
